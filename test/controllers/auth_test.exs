defmodule SummitChat.AuthTest do
  use SummitChat.ConnCase
  alias SummitChat.Auth

  setup %{conn: conn} do
    conn =
      conn
      |> bypass_through(SummitChat.Router, :browser)
      |> get("/")
  ...
  test "authenticate_user continues when the current_user exists", %{conn: conn} do
    conn =
      conn
      |> assign(:current_user, %SummitChat.User{})
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      conn
      |> Auth.login(%SummitChat.User{id: 123})
      |> send_resp(:ok, "")
